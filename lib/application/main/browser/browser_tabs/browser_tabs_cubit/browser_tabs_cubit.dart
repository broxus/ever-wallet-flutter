...
class BrowserTabsCubit extends Cubit<BrowserTabsCubitState> {
  ...
  factory BrowserTabsCubit(
      HiveSource hiveSource, SitesMetaDataRepository metaDataRepository) {
  ...
  BrowserTabsCubit._(this._hiveSource, this.metaDataRepository, this._tabsDto)
      : super(
          BrowserTabsCubitState.hideTabs(
              _tabsDto, _tabsDto.lastActiveIndex, _tabsDto.tabs.length),
        ) {
  ...
  void showTabs() => emit(
        BrowserTabsCubitState.showTabs(
            _tabsDto, _tabsDto.lastActiveIndex, _tabsDto.tabs.length),
      );
  ...
  Future<void> openNewTab([String? url]) async {
    ...
    if (url == null) {
      tab = const BrowserTab(
          url: aboutBlankPage, image: '', title: '', lastScrollPosition: 0);
    } else {
  ...
  Future<void> updateCurrentTab(String url) async {
    ...
    final tab = BrowserTab(
        url: url, image: meta.image, title: meta.title, lastScrollPosition: 0);
  ...
  Future<void> updateCurrentTabData(
      int? scrollPosition, Uint8List? screenshot) async {
  ...
  Future<SiteMetaData> _getSiteMetaData(String url) async {
    try {
      ...
      logger.e('Failed to load tab MetaData', error: e, stackTrace: t);
    }
